...
1. With Python 3.10+ installed, run `python scripts/setup-blender-mcp.py` from the repository root. It installs only into ignored `.local/blender-mcp/`. Python 3.14.6 is verified here. The pinned source is fixed; transitive Python package versions are resolved during installation.
...
3. Open this trusted project in Codex and restart its MCP connection if needed. [Official Codex MCP documentation](https://developers.openai.com/codex/mcp/) supports project-local `.codex/config.toml`; no global Codex configuration is changed. The server command assumes the project root as its working directory.
