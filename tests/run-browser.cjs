  ...
  try {
    if (suite === 'performance') {
      ...
      for (const mode of ['on','off']) {
        await page.goto(`http://127.0.0.1:4173/?renderer=${renderer}&vfx=${mode}`);
        report[mode] = await run(page);
        console.log(JSON.stringify({mode,...report[mode]}));
      }
    } else {
      await page.goto(`http://127.0.0.1:5173/?renderer=${renderer}`);
      for (const name of ['vfx','gameplay']) {
        report[name] = await readCheck(name)(page);
        console.log(`${name} passed ${channel} ${renderer}`);
      }
    }
    fs.writeFileSync(`output/playwright/c005-${channel}-${renderer}-${suite}.json`, JSON.stringify(report,null,2));
  } finally {
